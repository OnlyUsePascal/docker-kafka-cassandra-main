...
CASSANDRA_HOST = get_env("CASSANDRA_HOST", 'localhost')
CASSANDRA_KEYSPACE = get_env("CASSANDRA_KEYSPACE", 'kafkapipeline')
WEATHER_TABLE = get_env("WEATHER_TABLE", 'weatherreport')
FAKER_TABLE = get_env("FAKER_TABLE", 'fakerdata')
CUSTOM_TABLE = get_env("CUSTOM_TABLE", 'customdata')
...
# FAKER_TABLE = os.environ.get("FAKER_TABLE") if os.environ.get("FAKER_TABLE") else 'fakerdata'
# CUSTOM_TABLE = os.environ.get("CUSTOM_TABLE") if os.environ.get("CUSTOM_TABLE") else 'customdata'
...
#         counter += 1
#         if counter >= 100:
#             print('inserting ' + sdef saveTwitterDf(dfrecords):
#     if isinstance(CASSANDRA_HOST, list):
#         cluster = Cluster(CASSANDRA_HOST)
#     else:
#         cluster = Cluster([CASSANDRA_HOST])
...
#     print('Inserted ' + str(totalcount) + ' rows in total')tr(counter) + ' records')
#             totalcount += counter
#             counter = 0
#             batches.append(batch)
#             batch = BatchStatement(consistency_level=ConsistencyLevel.QUORUM)
